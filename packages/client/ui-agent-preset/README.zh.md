...
agent preset 的各个表层：General 设置中的一行，用于选择新建会话据以组装的 [preset](../../preset/agent-presets/README.md)；新建会话界面上的一枚 chip，用于选择**下一个会话**的 preset；会话标题旁的一个可切换标签；以及一个设置页分区，用于管理名单——复制、删除、默认值，以及通往 preset 自身文件的入口。

## 新建会话的选择与标题切换的区别

会话在创建时由某份 preset 组装，宿主拒绝以不同 preset **接管**已存在的会话——该会话的历史是在最初那份 preset 的工具下产生的。因此 General 行不可能是实时切换，它也如实说明了这一点：更改只对此后开启的会话生效。

会话标题旁的标签是实时切换。它以另一份 preset 重组正在运行的 agent，改动从下一回合开始生效：更早的回合继续按其记录的组装渲染，切换也会落盘，因此恢复的会话会重建它最后运行的那份 preset。
...
已经开始的会话并不是 chip 正在为其选择的那一个，因此暂存值会被丢弃而非排队；切换那种会话走标题旁的标签。

## 会话标题旁的标签

第三个表层，位于会话标题旁：**本会话**所运行的 preset，可通过其他选择器共用的同一份名单菜单切换。它从会话自身的摘要读取 preset，并在 General 行所读的同一份名单上解析显示名称。转发的 owner 事件 `agent-preset/selected` 会在每个标签页中把已经提交的切换折进这份共享摘要；发起方标签页可能已经采用 RPC 回执，而合并是幂等的。
