...
第一版支持粘贴和拖放 PNG、JPEG、WebP 与 GIF，支持仅图片或混合提示词，支持渲染历史用户图片与助手图片、单击预览原图，以及从缩略图和预览上下文菜单复制或保存图片（展示与交互细节部分由[附件展示对齐 Note](2026-08-11-web-attachment-display-alignment.md)取代）。文件选择、通用文件、PDF、音频和视频仍分别作为后续工作。
...
- 图片操作菜单复制的是图片字节，而不是消息文字。桌面端会先将载荷规范化为 PNG，并验证原生剪贴板确实包含图片；浏览器端使用图片 Clipboard API。
...
附件包构成一个能力 seam 的接口与实现侧。输入区行为留在会话对象层，提供方转换留在适配器中，无需修改 `agent-loop`。桌面端打包会把工作区构建出的 Host API proxy 覆盖到锁定的运行时中，使 Host 准入检查与发布的前端使用同一份源码构建。

### 实现

已实现的范围包括附件 seam、角色无关的图片块、Pi-AI 输入转换、DeepSeek 拒绝、宿主持久化顺序、Web 上传与读取协议、当前图片限制执行、大小受限的 Web 请求体、内存草稿图片、粘贴与拖放附件栏、用户与助手历史图片渲染、图片操作菜单、单击预览、压缩处理，以及组装后无需密钥的 Web 覆盖。Windows 桌面端打包会在安装锁定的运行时依赖后，覆盖工作区构建出的 Host API proxy。
...
- 文件选择、通用文件与 PDF、音频与视频、持久草稿暂存、输出提供方认证和按引用感知的垃圾回收仍是相互独立的设计。
